...
class MonitorableResource
  ...
  def init_monitor_signals
    if @resource.respond_to?(:init_monitor_signals)
      @monitor_signals = @resource.init_monitor_signals
    else
     @monitor_signals = { pulse: Pulse.new(@resource) }
    end
  end
  ...
  def check_health
    @run_event_loop = true if @resource.needs_event_loop_for_pulse_check?
    @pulse_check_started_at = Time.now
    @monitor_signals.each do |name, signal|
      @health[name] = signal.get_status(session: @session, prev: @health[name])
    end
